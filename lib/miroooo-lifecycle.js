...
const PRODUCTS = {
  "1000000675113473": { model: "X1", name: "Miroooo X1", handle: "miroooo-x", image: CDN + "2fea3bd3-c11d-4352-a4a9-b4ebb41b5184.jpeg", heads: false },
  "1000000675072187": { model: "X2", name: "Miroooo X2", handle: "miroooo-x2", image: CDN + "938f5342-b52f-44a9-b64c-0a5f5a608bdf.jpeg", heads: false },
  "1000000675471182": { model: "X1", name: "Miroooo X1 Heads", handle: "miroooo-x1-heads", image: CDN + "db077ed0-3c99-4536-b7ca-745010b55268.jpeg", heads: true },
  "1000000675616058": { model: "X2", name: "Miroooo X2 Heads", handle: "miroooo-x2-heads", image: CDN + "268d9b15-c592-4ab7-87e8-961a673cdcce.jpeg", heads: true }
};
const VARIANTS = {
...
async function profileForEmail(email) {
  ...
  // Search results can lag recent consent/property changes; read the profile itself.
  const fresh = await klaviyo("profiles/" + response.data[0].id + "/?additional-fields[profile]=subscriptions");
  return fresh.data;
}
...
async function readCheckout(checkoutUrl) {
  ...
  // ShopBase also returns a structured `token`; do not let it replace our URL token.
  return { ...result.result, token, url: url.origin + url.pathname };
}
...
async function processLifecycle(body) {
  ...
  if (body.kind === "checkout") {
    ...
    if (profile.attributes.properties?.miroooo_recovery_completed_checkout === checkout.token) return { status: "recovery_already_completed" };
