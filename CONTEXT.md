...
- `faq.html`: FAQPage schema route. Key promises include 1-3 business days processing, 3-10 business days tracked transit, 90-day home trial, two-product range difference, and support via `support@trymirooo.com`.
...
## 2026-09-04 17:29:58 +05:30 - Exact X2 cart-to-checkout promo reconciliation and live Buy 2 rule correction

- Repository/domain state: `E:\1st YEAR DTU\New folder\gobrush-product-page`; storefront `https://www.trymiroooo.com` / `https://trymiroooo.com`; public checkout `https://miroooo.us`; PlusBase cart/admin origin `https://muuhu.onshopbase.com`; GitHub remote `https://github.com/naman-14113114/gobrush-product-page.git`.
- Starting Git state: branch `main`, HEAD `f1b559cf79bcf1b3d4a9039456a753169a38d39e`, upstream `origin/main` at the same commit, ahead/behind `0/0` after `git fetch --all --prune`, and a clean worktree. The latest commits after the previous checkout work changed product descriptions/reviews and did not overlap checkout code. No pull was needed.
- User request and practical meaning: refresh from the latest local/GitHub state, reproduce the exact X2 sequence where Buy 2 checkout is followed by returning to cart and switching to Buy 3, then make checkout contain exactly the promos currently present in cart. Buy 3 must replace Buy 2 promos rather than stack with a stale `2-BRUSH-BUNDLE-SPECIAL`. The user explicitly authorized use of the supplied PlusBase access for this correction; credentials were not copied into source, context, logs, or environment files.
- Scope and protected areas: only the X2 promo/checkout contract, the one conflicting PlusBase discount rule, and server-side promo verification were changed. Product layouts, X1/X2 content, images, videos, prices, item IDs, bundle quantities, cart calculations, reviews, tracking, SEO, policy pages, domains, other discount rules, orders, and payments were protected.
- Files/docs inspected: workspace `AGENTS.md` and `CONTEXT.md`; repo `CONTEXT.md`, `README.md`, `PRODUCT.md`, `DESIGN.md`, `DESIGN.json`, `api/checkout/prepare.js`, `cart.html`, and `assets/site.js`; current Git status/remotes/log/diffs; ShopBase automatic/manual discount documentation and price-rule API shape; live PlusBase price-rule and discount-code records; production checkout prepare responses; authoritative `https://miroooo.us/api/checkout/<token>/next/new-info.json`; `next/apply-coupon.json`; `next/delete-coupon.json`; and visible production storefront/cart/checkout pages through Playwright.
- Corrected root cause:
  - Cart state was already correct: quantity 2 sends `2-BRUSH-BUNDLE-SPECIAL` plus `FREE2HEADS`, while quantity 3 removes those and sends `3-BRUSH-BUNDLE-OFFER` plus `FREE4HEADS`. The prepare route also creates a distinct PlusBase cart and checkout token on every call, so browser cache, cookies, and token reuse were not the cause.
  - A fresh Buy 3 checkout with no requested promo codes nevertheless contained `2-brush-bundle-special` in authoritative `result.discounts`. A separate fresh Buy 3 request containing only the two Buy 3 codes returned all three codes, yet the endpoint reported success because it checked only for missing requested codes and ignored unexpected active codes.
  - Live PlusBase configured `2-brush-bundle-special` as the only active automatic Miroooo bundle rule, with a minimum eligible X2 quantity of 2 and no maximum. That means Buy 3 also qualifies. The Buy 3 bundle rule and the free-head rules are manual/cart-controlled.
  - Calling `next/delete-coupon.json` for the Buy 2 code returned success, but the next authoritative checkout refresh reapplied it automatically. This proved that removing/re-adding coupons or clearing browser cookies could not permanently solve the issue while the rule remained automatic.
  - ShopBase's quantity prerequisite supports a minimum threshold, not an exact quantity or maximum threshold. Therefore the durable correction is for the cart/API to be the sole authority by making the Buy 2 code manual, matching the existing Buy 3 flow.
- Live PlusBase Admin change: updated only the existing `2-brush-bundle-special` price rule type from `automatic` to `manual`. A before/after field comparison showed that only `type` and `updated_at` changed. The title/code, active status, `-7.25` percentage value, minimum quantity 2, three eligible paid X2 variants, start/end dates, and combination data remained unchanged. A final Admin read confirmed the rule is active/manual and its existing `2-brush-bundle-special` discount-code record remains present.
- Source file changed: `api/checkout/prepare.js`. It now derives `unexpectedDiscountCodes` from authoritative active checkout discounts for all known Miroooo promo codes. The prepare endpoint returns HTTP 502 with `missingDiscountCodes` and `unexpectedDiscountCodes` whenever PlusBase's known promo set differs from the cart-requested set; successful responses also expose an empty `unexpectedDiscountCodes` diagnostic. This prevents a future automatic-rule/config regression from producing another false success redirect.
- Context files changed: append-only repo `CONTEXT.md` and append-only workspace `E:\1st YEAR DTU\New folder\CONTEXT.md`. `npm run build` regenerated ignored `public/` output; no tracked generated storefront file changed.
- Live/API verification:
  - Fresh Buy 2 with two X2 brushes, one X2-head set, and only `2-BRUSH-BUNDLE-SPECIAL,FREE2HEADS` returned and authoritatively contained exactly those two codes.
  - Fresh Buy 3 with three X2 brushes, two X2-head sets, and only `3-BRUSH-BUNDLE-OFFER,FREE4HEADS` returned and authoritatively contained exactly those two codes, with no Buy 2 code.
  - Fresh Buy 3 with no requested codes contained no active discount codes, proving the automatic overlap was removed.
  - Full production Playwright reproduction passed: select default Buy 2 on X2, add to cart, checkout visibly lists only `2-brush-bundle-special` and `FREE2HEADS`; navigate back to the same cart, increase quantity from 2 to 3, observe cart promos change to `3-BRUSH-BUNDLE-OFFER` and `FREE4HEADS`; proceed again to a new checkout, which visibly lists only `3-brush-bundle-offer` and `FREE4HEADS`.
- Local verification: `node --check api/checkout/prepare.js`, `git diff --check`, `npm run build`, and `npm run verify` passed; the verifier checked 43 required files and 17 storefront pages. Direct invocation of the edited handler against live checkout returned HTTP 200 with the exact Buy 3 set and no missing/unexpected codes. A mocked authoritative response containing the two requested Buy 3 codes plus the unwanted Buy 2 code returned HTTP 502 with `unexpectedDiscountCodes: ["2-BRUSH-BUNDLE-SPECIAL"]`.
- Mistakes/corrections during this task: the first live prepare verification used the non-`www` storefront URL; its canonical redirect converted POST to GET and returned `Method not allowed`, so verification was rerun directly against `https://www.trymiroooo.com/api/checkout/prepare`. The first mocked negative response omitted PlusBase's top-level `code: 200`, correctly triggering initialization failure rather than the intended unexpected-code branch; the fixture was corrected and the guard then produced the expected 502. One combined recursive Playwright cleanup command was blocked before execution; generated files were deleted individually with `apply_patch`, leaving no untracked test artifacts. The empty `.playwright-cli` directory remains untracked-invisible and contains no files.
- Tests/actions not performed: no customer identity, address, payment data, or order was submitted; no checkout was completed. No X1 promo behavior was changed. No Vercel deployment or production source promotion was performed, so the server-side exact-set guard is currently an unstaged local source change; the PlusBase rule correction itself is already live and is what fixes the current production Buy 2-to-Buy 3 overlap.
- Final Git/publishing state: branch `main` and `origin/main` remain at `f1b559cf79bcf1b3d4a9039456a753169a38d39e`, ahead/behind `0/0`; intended unstaged changes are `api/checkout/prepare.js` plus append-only context files. No commit, push, pull, branch, pull request, merge, rebase, stash, reset, Vercel deploy, domain/alias/environment change, payment, or order occurred. The only external production mutation was the explicitly required PlusBase Buy 2 rule type change described above.
- Remaining uncertainty/future start point: checkout sessions created before the rule conversion can retain their already-attached old discount state, but every new checkout generated by the cart now receives the exact current cart promo set. Future work should preserve the Buy 2 rule as manual and publish the local exact-set guard only when the user explicitly requests commit/push/deployment.

## 2026-09-06 07:50:00 +05:30 - MIROOOO Promo Code & Net Brush Amount Discount Update

- User request:
  - Update memory to latest from GitHub and local.
  - Support promo code `MIROOOO` (with backward compatibility for `MIROOOO10`).
  - Calculate promo discount against the **net brush amount before bundle discount** (e.g. £69 per brush) rather than the post-bundle discounted amount:
    - Buy 1 Brush: 10% on £69 = £6.90 -> rounded to whole integer `-£7` (Subtotal £62).
    - Buy 2 Brushes: 10% on £138 (69 + 69) = £13.80 -> rounded to whole integer `-£14` (Subtotal £114, after £10 bundle promo discount).
    - Buy 3 Brushes: 10% on £207 (69 + 69 + 69) = £20.70 -> rounded to whole integer `-£21` (Subtotal £156, after £30 bundle promo discount).
    - Buy N Brushes: 10% on `N * 69` = `Math.round((N * 69) * 0.10)`.
    - Brush X1: 10% on `qty * 69` = `Math.round((qty * 69) * 0.10)`.
    - Replacement Heads: 10% on `qty * 10` = `Math.round((qty * 10) * 0.10)`.
  - Strictly whole integer prices without decimals (`Math.round`).
  - Minimal changes, exact calculations, verified visually in browser.
- Files changed:
  - `cart.html`:
    - Updated `VALID_PROMOS` to include `"MIROOOO"`.
    - Updated initial `cartState` filtering to keep either `"MIROOOO"` or `"MIROOOO10"`.
    - Updated `computeTotals(state)` to track `netBrushAmount` (`qty * 69` for brushes, `qty * 10` for heads) and calculate `promoDiscount = Math.round(netBrushAmount * 0.10)`.
    - Updated `renderCartView()` breakdown row to display `PROMO (${promoName})` whenever `totals.promoDiscount > 0` for all products (X1, X2, heads).
    - Updated `initPromoCodeHandler()` to accept `MIROOOO` and `MIROOOO10`, replacing either if already entered and updating localStorage.
  - `api/checkout/prepare.js`:
    - Added `"MIROOOO"` to `validDiscountCodes` alongside `"MIROOOO10"`.
  - `assets/site.js`:
    - Added `"MIROOOO"` to `validCodes` alongside `"MIROOOO10"`.
- Verification:
  - `node --check api/checkout/prepare.js` and `node --check assets/site.js` passed.
  - `npm run verify` passed with 43 required files and 17 storefront pages checked.
  - `npm run build` passed and regenerated `public/`.
  - Verified in Chrome DevTools MCP on live preview server:
    - Buy 1 X2: Subtotal £62 (Promo -£7).
    - Buy 2 X2: Subtotal £114 (Bundle promo -£10, Heads -£10, Promo -£14, Total discount -£174).
    - Buy 3 X2: Subtotal £156 (Bundle promo -£30, Heads -£20, Promo -£21, Total discount -£281).
    - Buy 2 X1: Subtotal £114 (Promo -£14).
    - 2x Heads: Subtotal £18 (Promo -£2).
...
## 2026-09-22 09:22:00 +05:30 - Review Dates Shift by +14 Days for All Products

- Task: Shift review dates forward by 14 days across all product reviews in `gobrush-product-page`.
- Script: `node update-review-dates.js 14`.
- Files Modified:
  - `assets_ref/miroooo-reviews.js` (Miroooo X1):
    - 63 ISO dates shifted by +14 days (e.g., `2026-09-07` -> `2026-09-21`).
    - 63 display dates shifted (e.g., `7 September 2026` -> `21 September 2026`).
    - 8 merchant reply dates shifted (e.g., `9 July 2026` -> `23 July 2026`, `30 June 2026` -> `14 July 2026`).
    - 1 base timestamp shifted (`2026-09-09T12:00:00Z` -> `2026-09-23T12:00:00Z`).
  - `assets_ref/miroooo-x2-reviews.js` (Miroooo X2):
    - 56 ISO dates shifted by +14 days (e.g., `2026-09-08` -> `2026-09-22`).
    - 56 display dates shifted (e.g., `8 September 2026` -> `22 September 2026`).
    - 6 merchant reply dates shifted (e.g., `22 March 2026` -> `5 April 2026`, `14 February 2026` -> `28 February 2026`).
    - 1 base timestamp shifted (`2026-09-09T12:00:00Z` -> `2026-09-23T12:00:00Z`).
- Verification:
  - `npm run verify` passed cleanly (43 required files and 17 storefront pages checked).
  - `npm run build` completed cleanly, syncing updated review assets to `public/assets_ref/`.
